...
def make_fair_matches(matches, results):
    new_matches = []
    for _, game_history, match_id, depth in results:
...
def play_matches(custom_agent, test_agent, num_rounds, num_procs=1, fair_matches=True):
    ...
    my_depth = sum(r[3][0] for r in results)/len(results)
    oppo_depth = sum(r[3][1] for r in results)/len(results)
    wins = sum(int(r[0].name == custom_agent.name) for r in results)
    return wins, len(matches) * (1 + int(fair_matches)), my_depth, oppo_depth


def main(args):
    test_agent = TEST_AGENTS[args.opponent.upper()]
    custom_agent = Agent(CustomPlayer, "Custom Agent")
    wins, num_games, my_depth, oppo_depth = play_matches(custom_agent, test_agent,
    ...
    print("Your agent won {:.1f}% of matches against {}".format(
       100. * wins / num_games, test_agent.name))
    print("Your agent mean search depth is {:.2f}, your opponent mean search depth is {:.2f}".format(my_depth, oppo_depth))
    print()
